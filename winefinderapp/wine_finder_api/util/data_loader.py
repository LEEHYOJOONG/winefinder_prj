...
DATA_WINE = './static/vivino.xlsx'
DATA_WINE_IMAGE = './static/vivino_img_link.xlsx'
...
class WineDatasetLoader(SingletonInstance):
    def __init__(self):
        self._dataset: pd.DataFrame = pd.read_excel(DATA_WINE)
...
class WineImageLoader(SingletonInstance):
    def __init__(self):
        self._dataset: pd.DataFrame = pd.read_excel(DATA_WINE_IMAGE)
